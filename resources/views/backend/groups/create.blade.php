...
<div id="create" class="modal fade" tabindex="-3" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('groups.store') }}" id="add-group" method="POST" name="addGroupsForm" onsubmit="return validateForm()">
                <input name="_token" type="hidden" value="{{ csrf_token() }}">
                <input type="hidden" name="user_id" value="{{ Auth::id() }}">
                <div class="modal-body">
                    <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
                        <label class="control-label">Group Name</label>
                        <input type="text" class="form-control" id="name" name="name" onkeyup="removeErrors()">
                        @if ($errors->has('name'))
